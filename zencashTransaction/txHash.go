...
func (t Transaction) getHashesForSig(unlockData []TxUnlock, SegwitON bool, addressPrefix AddressPrefix) ([]TxHash, error) {
	...
	for i := 0; i < len(unlockData); i++ {
		...
		//	return nil, err
		//}
		lockBytes, _ := hex.DecodeString(unlockData[i].LockScript)

		sigBytes, err := t.getBytesForSig(lockBytes, nil, TypeP2PKH, unlockData[i].SigType, i, unlockData[i].Amount, SegwitON)
		if err != nil {
			return nil, err
		}
		...
		txHash, err := newTxHash(hash, lockBytes, nil, TypeP2PKH, unlockData[i].SigType, addressPrefix)
		if err != nil {
			return nil, err
		}
