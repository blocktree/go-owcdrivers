		...
		if len(signedTrans.Inputs[i].ControlProgram) == 0x22 {
			...
			for j := 0; j < len(sigs); j++ {
				for k := j; k < len(pubs); k++ {
					if owcrypt.SUCCESS == owcrypt.Verify(pubs[k], nil, txHashes[i], sigs[j], owcrypt.ECC_CURVE_ED25519_NORMAL) {
						count++
					}
				}
			}
			...
			if owcrypt.SUCCESS != owcrypt.Verify(signedTrans.Inputs[i].SigPub.Pubkey, nil, txHashes[i], signedTrans.Inputs[i].SigPub.Signature, owcrypt.ECC_CURVE_ED25519_NORMAL) {
				return false
			}
		}
	}
