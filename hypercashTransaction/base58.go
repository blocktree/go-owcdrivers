...
func DecodeCheck(address string) ([]byte, []byte, error) {
	...
	checksum := owcrypt.Hash(owcrypt.Hash(ret[:len(ret)-4], 0, owcrypt.HASH_ALG_BLAKE256), 0, owcrypt.HASH_ALG_BLAKE256)[:4]
	for i := 0; i < 4; i++ {
...
func EncodeCheck(prefix []byte, hash []byte) string {
	...
	checksum := owcrypt.Hash(owcrypt.Hash(data, 0, owcrypt.HASH_ALG_BLAKE256), 0, owcrypt.HASH_ALG_BLAKE256)[:4]
